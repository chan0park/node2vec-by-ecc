...
def build_neg_samples_multi(nodes, true_edges, num):
    false_samples = []
    while len(false_samples) < len(true_edges):
        sample_nodes = random.sample(nodes, 2)
        false_edge = [min(sample_nodes), max(sample_nodes)]
...
def simulate_walk_popularity_multi(args, G, nodes):
    if args.popwalk == "none":
        G.preprocess_transition_probs()
        walks = node2vec_walk_multi(args.num_walks, args.walk_length, G, nodes)
    elif args.popwalk == "pop":
        G.preprocess_transition_probs_popularity()
        walks = node2vec_walk_multi(args.num_walks, args.walk_length, G, nodes)
    elif args.popwalk == "both":
        G.preprocess_transition_probs()
        num_walks = int(args.num_walks/2)
        walks = node2vec_walk_multi(num_walks, args.walk_length, G, nodes)      
        G.preprocess_transition_probs_popularity()
        walks.extend(node2vec_walk_multi(num_walks, args.walk_length))
    return walks
...
def preprocess_transition_probs(nx_G, is_directed):
    '''
...
def main(args):
    ...
    print('single: '+str(time.time()-_time))
    _time = time.time()
    G.preprocess_transition_probs()
    num_pool = 9
    ...
    num_node = len(nodes)
    pool = Pool(num_pool)
    walks = pool.map(G.node2vec_walk, [args.walk_length]*num_node, nodes)

    alias_nodes, alias_edges = preprocess_transition_probs(nx_G, args.is_directed)
    num_pool = 9
    nodes = list(nx_G.nodes())
    num_node = len(nodes)
    pool = Pool(num_pool)
    ...
    # print(len(splited_nodes))
    # # raise
    # walks = []
    # walks = p.map(simulate_walk_popularity_multi, [args]*num_pool, [G]*num_pool, splited_nodes)
    # walks = [w for walk in walks for w in walk]
    print('multi: '+str(time.time()-_time))
    _time = time.time()
    # for node in nodes:
    #     walks = simulate_walk_popularity_multi(args, G, node)
    ...
    nodes = nx_G.nodes()
    neg_edges = build_neg_samples(nodes, test_edges)
    neg_edges = p.map(build_neg_samples_multi, [nodes]*num_pool, [test_edges]*num_pool, )
    build_neg_samples_multi(nodes, test_edges, num)
